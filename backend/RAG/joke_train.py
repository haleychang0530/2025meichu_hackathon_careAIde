...
# ====== 工具函式 ======
def jk_load_md_chunks(md_dir, max_chars=500):
    """
...
def jk_build_index(texts):
    embeddings = [jk_get_embedding(t) for t in texts]
...
if __name__ == "__main__":
    ...
    texts = jk_load_md_chunks(MD_DIR)
    print(f"共讀取 {len(texts)} 個 chunks")

    print("建立 FAISS Index...")
    index, texts = jk_build_index(texts)
    print("完成 ✅")
    
    faiss.write_index(index, "jk_kb_index.faiss")
    with open("jk_kb_text.pkl", "wb") as f:
        pickle.dump(texts, f)
